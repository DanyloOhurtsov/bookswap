...
describe('AnalyticsService.record (§3, §6)', () => {
   ...
   * підставлений сюди user id чи секрет не має права опинитися в логах як є.
   * Лейбл падає на літерал `UNKNOWN`, бо рядок не входить у сім дозволених типів.
   */
  it('невідомий type із умовним user ID/секретом не потрапляє в warning', async () => {
    const { service, createMany } = createService()
    const warn = jest.spyOn(Logger.prototype, 'warn').mockImplementation(() => undefined)

    const secret = 'user-id-cmxsecret-42-do-not-log'
    const invalid = {
      type: secret,
      subjectUserId: 'user-1',
      domainEntityId: 'copy-1',
