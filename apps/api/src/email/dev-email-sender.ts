 ...
 *
 * Тому відмова — на старті застосунку, а не на першому надсиланні: краще
 * впертися в помилку деплою, ніж дізнатися про це від користувача.
 */
...
export class DevEmailSender implements EmailSender {
  ...
  constructor(private readonly config: ConfigService) {
    assertNotProduction(this.nodeEnv)
  }

  send(message: EmailMessage): Promise<void> {
    // Друга перевірка, вже на гарячому шляху. Конструктор ловить звичайний
    // випадок — старт застосунку; ця страхує від екземпляра, створеного повз
