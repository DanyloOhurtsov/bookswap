...
> **Пріоритети й release scope:** цей документ зберігає технічні вимоги та
> інваріанти. Актуальний порядок реалізації визначає
> [Product Roadmap v2](./plan/roadmap-v2.md). Історична черга із §14 більше не є
> активним roadmap.
...
### 6.7 Відгуки й оцінки — відкладений модуль

> Не входить до Beta або Public v1 за Product Roadmap v2. Нижче збережено
> технічні вимоги на випадок, якщо модуль пройде окремий продуктовий gate.
...
| Видання     | _поза v1_           | Папір, шрифт, палітурка |
...
### Оцінки — відкладено після Public v1
...
Ці маршрути описують можливий майбутній API й не входять до активного v1 scope.
...
## 10. Ранг перекладів — відкладений технічний дизайн

Цей розділ не задає поточний пріоритет реалізації. Структуровані ознаки
перекладів і наявність конкретного перекладу у друзів розвиваються без
соціальних ratings; формула повертається в backlog лише після продуктового gate.
...
## 14. Історична технічна послідовність

> Етапи 1–4 нижче вже виконані. Колишній етап 5 з ratings відкладено. Активні
> наступні етапи 8+ описані в [Product Roadmap v2](./plan/roadmap-v2.md); цей
> розділ не слід використовувати для вибору наступної роботи.
...
**Колишній етап 5 — оцінки, відкладено.** `Review` на твір, `TranslationRating` і ранг перекладів із cold-start-правилом збережені як можливий майбутній модуль. Структуровані ознаки перекладів залишаються чинною частиною каталогу.
...
Історичні залежності були такими:

- Етап 3 після 2, бо сповіщати нема про що, поки немає переходів.
- Якщо ratings колись повернуться, вони все одно залежать від уже виконаної дедуплікації каталогу: інакше оцінки розмажуться по дубліктах.
