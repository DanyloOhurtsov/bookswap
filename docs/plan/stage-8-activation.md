...
**Статус:** execution plan, рішення затверджено; блокерів реалізації немає.
...
- **8a-4** — CLI `funnel:report` повністю: `--from/--to/--window-days/--json`,
  точні тексти §6.1 і §6.3, «not instrumented» для 5–6, тимчасові метрики North
  Star з визначенням active user (§6), cross-check event-only/domain-only.
...
## 14. Рішення затверджено; блокерів реалізації немає
...
`occurredAt`). 8a-1–8a-3 реалізовано (§12). Наступний крок — підетап 8a-4 за окремим
запитом.
